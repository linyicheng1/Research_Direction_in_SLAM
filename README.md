...
>     5. 信息处理（特征点）—— 特征点关联估计位姿
>     6. 信息处理（后端优化）—— 基于关键帧结构的后端优化
>     7. 信息处理（地图）—— 地图构建
>     8. 端到端的多任务处理
> 3. 视觉SLAM相关的其他任务
...
果冻效应不仅仅表现在图像像素位置的移动，还会导致每一行的图像成像曝光参数不一致，即整张图像不是由统一的参数采集得到，导致灰度一致性、或者光度一致性差。

|研究问题|动机|创新点|难度|详细信息|
|---- | ---- | ---- | ---- | ---- |
|卷帘相机与光度不变性冲突|基于卷帘相机中的不变量改进光流/直接法|卷帘相机中的直接法|:star::star::star::star:|
...
基于特征点的关联求解局部BA问题，或者仅优化位姿问题非常的成熟和有效，但也涌现了一些基于神经网络的求解器，能够更快速的求解复杂问题。

|研究问题|动机|创新点|难度|详细信息|
|---- | ---- | ---- | ---- | ---- |
|新求解器应用|基于神经网络的方程求解器改进局部BA|考虑CVPR2022最佳论文之类的神经网络求解方法，将传统SLAM中初始化或者局部BA方法替换掉，提高计算速度|:star::star::star::star::star:|
...
由于直接法对光度一致性有较高的要求，这并不总是满足，因此对其光照性能的改进是重要的一环。

|研究问题|动机|创新点|难度|详细信息|
|---- | ---- | ---- | ---- | ---- |
|光照鲁棒的直接法|削弱过强的光度一致性的假设||:star::star::star::star::star::star:|
